...
async function processImages() {
  ...
  for (const cat of categories) {
    for (const tmpl of templates[cat]) {
      ...
      try {
        const success = await downloadViaHttps(url, filepath);
        if (success) {
          console.log(
            `[${ptId}/${total}] Downloaded Picsum OK for ${tmpl.term}`,
          );
        } else {
          console.log(`[${ptId}/${total}] FAILED to download ${tmpl.term}`);
        }
      } catch (err) {
        console.log(`[${ptId}/${total}] ERROR for ${tmpl.term}`);
      }
