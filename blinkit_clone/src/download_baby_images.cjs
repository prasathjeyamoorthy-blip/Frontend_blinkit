...
const getImageUrl = (query) => {
  return new Promise((resolve, reject) => {
    https
      ...
      .on("error", () =>
        resolve(
          "https://via.placeholder.com/400?text=" + encodeURIComponent(query),
        ),
      );
  });
};
    ...
    client
      .get(url, (res) => {
        if (res.statusCode === 200) {
          ...
          // Download failed, create a placeholder using Jimp or just copy a default
...
async function processImages() {
  ...
  for (const cat of categories) {
    for (const tmpl of templates[cat]) {
      ...
      if (!success) {
        console.error(
          `=> FAILED downloading image for ${tmpl.term}. Proceeding anyway.`,
        );
      }
